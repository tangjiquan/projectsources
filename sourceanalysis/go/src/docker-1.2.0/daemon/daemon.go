...
1.向eng对象中注册众多的处理方法对象
2.daemon.Repositories().Install(eng)实现了向eng对象中注册多个与docker镜像相关的Hanlder
...
1、应用配置信息：
	...
	4.处理PID文件配置
2、检查系统支持及用户权限
	1.操作系统类型对Docker Daemon的支持
 	2.用户权限的级别
	3.内核版本与处理器的级别

3、配置工作路径
	1.使用规范路径创建一个TempDir，路径名为tmp
	2.通过tmp，创建一个指向tmp的文件路径符合连接realTmp
	3.使用realTemp的值，创建并赋给环境变量TEMPDIR
	4.处理config的属性EnableSelinuxSupport
	5.将realRoot重新赋值于config.Root，并创建Docker Daemin的工作
...
func NewDaemonFromDirectory(config *Config, eng *engine.Engine) (*Daemon, error) {
	...
	repository:     存储所有Docker容器信息的路径，默认为/var/lib/docker/containers
	containers:     用于存储docker容器信息的对象
	graph:          存储Docker镜像的graph对象
	repositories:   存储本机所有Docker镜像repo信息的对象
	idIndex:        用于通过简短有效的字符串前缀定位唯一的镜像
	sysInfo:        系统功能新
	volumes:        管理宿主机上volumes内容的graphdiver，默认为vfs类型
	config:         Config.go文件中的配置信息，以及执行后产生的配置DisableNetwork
	containerGraph: 存放Docker进行关系的graphdb
	driver:         管理docker镜像的驱动graphdriver，默认为aufs类型
	sysInitPath:    系统dockerinit二进制文件所在的路径
	execDriver:     Docker Daemon的exec驱动，默认为native类型
	eng:            Docker的执行引擎Engine类型
