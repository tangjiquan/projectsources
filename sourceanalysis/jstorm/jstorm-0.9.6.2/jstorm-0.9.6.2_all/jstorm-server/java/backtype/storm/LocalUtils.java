...
public class LocalUtils {
	...
	public static LocalClusterMap prepareLocalCluster() {
		...
		try {
			...
			Map nimbusConf = deepCopyMap(conf);//将上面初始化的数据深度拷贝一份
			...
			state.setNimbus(instance.launcherLocalServer(nimbusConf,
					new DefaultInimbus()));//使用nimbusserver创建一个serverhandler
			state.setZookeeper(zookeeper);
			state.setConf(conf);
			state.setTmpDir(tmpDirs);
			state.setSupervisor(supervisor
					.mkSupervisor(supervisorConf, context));//使用supervisor创建supervisormanger
			return state;
		} catch (Exception e) {
	...
	private static Map getLocalConf(int port) {
		...
		conf.put(Config.STORM_CLUSTER_MODE, "local");//设置本地集群模式
		conf.put(Config.STORM_ZOOKEEPER_SERVERS, zkServers);//设置zk集群
		conf.put(Config.STORM_ZOOKEEPER_PORT, port);//设置zk集群的端口
